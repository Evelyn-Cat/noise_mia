...
MGGradientDescentOptimizer = tf.train.GradientDescentOptimizer


def make_vectorized_optimizer_class(cls):
    """Given a subclass of `tf.compat.v1.train.Optimizer`, returns a vectorized MGDP subclass of it.

    Args:
      cls: Class from which to derive a MG DP subclass. Should be a subclass of
        `tf.compat.v1.train.Optimizer`.

    Returns:
      A MG DP-SGD subclass of `cls`.
    """
    ...
    class MGDPOptimizerClass(cls):  # pylint: disable=empty-docstring
        __doc__ = ("""The following instrucion is not modified for MG mechansim so far.
                  
                  Vectorized DP subclass of `{base_class}` using MG
            averaging.

            You can use this as a differentially private replacement for
            `{base_class}`. This optimizer implements DP-SGD using
            the standard MG mechanism. It differs from `{dp_class}` in that
            ...
            """).format(
                ...
                dp_class='DP' +
                cls.__name__.replace('Optimizer', 'MGOptimizer'),
        ...
        def __init__(
              ...
              **kwargs):
            """Initialize the MGDPOptimizerClass.

            Args:
              l2_norm_clip: Clipping norm (max L2 norm of per microbatch gradients).
              noise_multiplier: MG noise epsilon threshold under given DEFAULT_DELTA.
            ...
            """
            super(MGDPOptimizerClass, self).__init__(*args, **kwargs)
            if isinstance(noise_multiplier, list):
              self._distributions = noise_multiplier[0]
              self._noise_multiplier = noise_multiplier[1]
            elif isinstance(noise_multiplier, (int, float)):
        ...
        def compute_gradients(self,
                              ...
                              grad_loss=None,
                              gradient_tape=None):
          """MG DP-SGD version of base class method."""
          if callable(loss):
            ...
            def process_microbatch(microbatch_loss):
                ...
                grads, _ = zip(*super(MGDPOptimizerClass, self).compute_gradients(
                    microbatch_loss,
                    var_list,
                    gate_gradients,
                    aggregation_method,
            ...
            def reduce_noise_normalize_batch(stacked_grads):
                ...
                print(f"NG noise: {self._distributions} {self._noise_multiplier}")
                
                if self._distributions is None:
                  noise = generate_noise(self._noise_multiplier, DEFAULT_DISTRIBUTIONS, tf.shape(input=summed_grads))
                else:
                  noise = generate_noise(self._noise_multiplier, self._distributions, tf.shape(input=summed_grads))
    ...
    return MGDPOptimizerClass


VectorizedMGDPSGDOptimizer = make_vectorized_optimizer_class(
    MGGradientDescentOptimizer
)
VectorizedMGDPSGD = VectorizedMGDPSGDOptimizer

VectorizedDPSGD = VectorizedMGDPSGD