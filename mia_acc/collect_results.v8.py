...
for suffix_epochs in Ts:
    noise_type = "mg"
    filename = f"{version}.mg.0.{suffix_dataset}.{suffix_model}.T_{suffix_epochs}.log"
    ...
    try:
        ...
        cfg.loc[cnt, ["noise_type", "mia", "acc", "T", "version", "machine"]] = ["mg", float(accuracy_mia), float(accuracy_maintask), int(suffix_epochs), version, machine]
        cnt = cnt + 1
    except:
        if "exists" in rs[-1]:
    ...
    for sigma in [0.01, 0.05, 0.1, 0.5, 1, 2, 5, 10, 100, 1000, 10000]:
        filename = f"{version}.gaussian.{sigma}.{suffix_dataset}.{suffix_model}.T_{suffix_epochs}.log"
        filepath = os.path.join(savefolder, filename)
        with open(filepath, 'r', encoding='utf8') as f:
            rs = f.readlines()
        ...
        try:
            ...
            cfg.loc[cnt, ["noise_type", "mia", "acc", "T", "version", "machine"]] = [sigma, float(accuracy_mia), float(accuracy_maintask), int(suffix_epochs), version, machine]
            cnt = cnt + 1
        except:
            if "exists" in rs[-1]:
                print(f"please run the inference again for {filename}")
