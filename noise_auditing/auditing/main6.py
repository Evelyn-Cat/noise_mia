...
if __name__ == '__main__':
    ...
    for name in combined:
        if np.concatenate(combined[name]).ravel().shape != (0,):
            ...
            filename = os.path.join(res_dir, '-'.join(['all', noise_type] + list(name)))
            np.save(filename, np.concatenate(combined[name]).ravel())
            # print(np.concatenate(combined[name]).ravel())
    ...
    if len(all_files) == len(combined) and search_ct > 0:
        ...
        for cfg in valid_cfg[:1]:
            ...
            vals = {"nobkd": np.mean(nobkd_li), "bkd": np.mean(bkd_li)}
            print("noise_type: {}, noise: {}, pois_ct: {}, clip norm: 1, nobkd mi: {}".format(*cfg, np.mean(nobkd_li)))
            print("noise_type: {}, noise: {}, pois_ct: {}, clip norm: 1, bkd mi: {}".format(*cfg, np.mean(bkd_li)))
