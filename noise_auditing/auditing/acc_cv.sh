    ...
    for value in "${list2[@]}"; do
        for noise_param in "${noise_params[@]}"; do
            for T in "${Ts[@]}"; do
                ...
                python acc_cv.py -dataset ${param} -model ${value} -l2_norm_clip 1 -noise_type lmo -noise_params ${noise_param} -microbatches 25 -epochs $T -batch_size 250 > log.$param.$value.$noise_param.$T.log
            done
        done
    done
done
