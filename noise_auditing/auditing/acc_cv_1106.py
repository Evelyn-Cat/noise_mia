...
def main(_):
    if FLAGS.noise_type == "lmo":
        df = pd.read_excel("Qt.xlsx")
        print(df)
        epsilon, sigma, clip, q, G_k, G_theta = df.loc[int(FLAGS.noise_params), ["epsilon", "sigma", "clip", "q", "k", "theta"]]
        
        FLAGS.l2_norm_clip = clip
        params = {'a1': 1.0, "G_k": G_k, "G_theta": G_theta, 'gaussian': sigma, 'distributions': ["Gamma"]}
        print(f"ng noise params: {params}")
        
    
    dataset = FLAGS.dataset
    FLAGS.exp_name = FLAGS.dataset + "-" + FLAGS.model + "-" + FLAGS.noise_type + "-" + str(FLAGS.noise_params) + "-" + str(FLAGS.epochs) + "-" + str(FLAGS.l2_norm_clip) + '-' + str(q)
    ...
    dataset = "p100"
    data_dir = "/root/new_lmo/noise_mia/noise_auditing/auditing/datasets"
